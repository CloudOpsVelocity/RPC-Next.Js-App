...
const AmenitiesDisplay = ({ amenitiesData }: AmenitiesDisplayProps) => {
  ...
  const subCategories = Object.keys(amenitiesData[selectedCategory]);
    ...
    <div className="flex flex-col md:flex-row">
      ...
      <div className="flex bg-[#fff] md:ml-[17px] flex-col px-4 md:px-[43px] pt-3.5 pb-0 rounded-lg border-[0.5px] border-solid border-[#92B2C8] max-h-[400px] md:max-h-[500px] overflow-y-auto w-full">
        {subCategories.map((subCategory: string, index: number) => (
          <ul key={subCategory} className={`py-3 ${index !== subCategories.length - 1 ? 'border-b border-[#92B2C8]' : ''}`}>
            <li className="text-[#4D6677] text-lg md:text-xl font-semibold list-disc ml-3 mb-2">{subCategory}</li>
            <div className="flex flex-wrap gap-2">
              {amenitiesData[selectedCategory][subCategory].map((amenity: Amenity) => (
                <div
                  key={amenity.cid}
