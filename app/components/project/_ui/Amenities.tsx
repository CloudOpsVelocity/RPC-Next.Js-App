...
export default function AmenitiesDisplay({ amenitiesData, data }: AmenitiesDisplayProps) {
  ...
  >
    ...
    >
      <div
        className={`w-1 h-8 ${
          selectedCategory === category ? 'bg-green-600' : 'bg-gray-300'
        } rounded-full`}
      />
      <span
        className={`text-base font-semibold ${
          selectedCategory === category ? 'text-green-700' : 'text-gray-800'
        }`}
      >
        {category}
      </span>
    ...
    <ul className="ml-6 mt-2 space-y-1">
      ...
      <div className="hidden md:block bg-white flex-grow rounded-lg border border-gray-200 shadow-sm overflow-hidden" ref={mainDisplayRef}>
        <div className="h-[500px] overflow-y-auto">
          {getAvailableSubCategories(selectedCategory).map((subCategory: string, index: number) => {
            ...
            return (
              ...
              >
                <h3 className="text-gray-800 text-xl font-semibold mb-4">{subCategory}</h3>
