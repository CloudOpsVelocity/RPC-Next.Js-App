...
export default function RecentSearchedUnits({propCgId,recentFiltersClick,className}: Props) {
    ...
    recentUnits.length > 0 && (
        <div className={className ?? ''}>
          ...
          <ul className="flex flex-wrap gap-2">
            {recentUnits.reverse().map((unit: any, index: number) => {
              // Extract key-value pairs
              const entries = Object.entries(unit);
              
              // Get the first two non-null filters
              const firstTwoNonNullableEntries = entries.filter(([key, value]) => value !== null).slice(0, 2);
              
              // Prepare display label
              const label = firstTwoNonNullableEntries.map(([key, value]: any) => (
            <span key={key} onClick={()=>{
                  ...
                  {firstTwoNonNullableEntries.length > 1 &&
                    firstTwoNonNullableEntries.indexOf(firstTwoNonNullableEntries[firstTwoNonNullableEntries.length - 1]) < firstTwoNonNullableEntries.length - 1 && ' - '} {/* Add dash between first and second */}
                </span>
              ));
  ...
  Reverse To Previous Search 
                ...
                >
                  ...
                  >
                    ...
                    {hasMoreFilters && <span className="text-gray-500"> ...</span>} {/* Indicate more filters */}
                  </div>
                </Tooltip>
              );
            })}
