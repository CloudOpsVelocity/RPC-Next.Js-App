...
function createPropertyString(data: Main): string {
  if (
    ...
    return `${data.propName} | Unit No. ${data.unitNumber} | ${data.facingName} | Area. ${formatNumberWithSuffix(data.plotArea)} sq.ft`;
  } else if (data.propTypeName === config.propTypeNames[5]) {
    return `${data.propName} | ${data.bhkName} | Floor ${data.totalFloor} | Unit No. ${data.unitNumber} | ${data.facingName} | Area: ${formatNumberWithSuffix(data.sba)} sq.ft`;
  } else {
