...
function ContentBox({ heading, text, content, date, type }: Props) {
  return (
    <div className={`w-full ${type == "small" ? "p-[10px]" : ""} `}>
        <h3 className={`text-[color:var(--800,#2D3748)] not-italic font-bold leading-[normal] mb-[4px] ${type == "small" ? "text-[14px]" : "text-[24px] md:text-[18px]"} `}>{heading}</h3>
        <p className={`text-[#01417C] italic font-bold leading-[normal] ${type == "small" ? "text-[12px]" : "text-[16px] md:text-[14px]"} `}>{text}</p>
        {content &&
        <p className={`text-[#303030] not-italic font-normal leading-[normal] mb-[16px] mt-[16px] ${type == "small" ? "text-[12px]" : "text-[16px] md:text-[14px]"} `}>{content}</p>
        }

        <div className={`flex justify-between items-center `}>
            <p className={`text-[#627A9E] italic font-medium leading-[normal] ${type == "small" ? "text-[12px]" : "text-[16px] md:text-[14px]"} `}>{date}</p>
