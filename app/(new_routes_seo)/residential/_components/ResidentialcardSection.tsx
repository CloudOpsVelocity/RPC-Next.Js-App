  ...
  useEffect(() => {
    ...
    const observerCallback = (entries: any, observer: any) => {
      entries.forEach((entry: any) => {
        if (entry.isIntersecting && entry.target.id === `item-${items.length - 1}`) {
      ...
      {!properties || properties.length < 1 ? (
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {properties.slice(0, listItemsCount).map((property: any, index: any) => {
              if (!property) {
              ...
              const possessionDate = property.possassionDate
                ...
                : 'N/A';
              const propertyType =
                Array.isArray(property.propType) && property.propType.length > 0
              ...
              return (
                ...
                >
                  ...
                  <div className="p-6">
                    <Link
                      href={`/residential/projects/${property.city?.toLowerCase() || 'unknown'}/${
                        property.locality?.toLowerCase() || 'unknown'
                      }/${property.projName?.toLowerCase().replace(/ /g, '-')}-${property.projIdEnc}`}
                      className="text-xl font-bold mb-2 text-blue-600 hover:cursor-pointer"
                    >
                      {property.projName || 'Unnamed Property'}
                    </Link>
                    <p className="text-muted-foreground flex items-center gap-2 mb-4">
                      <FaMapMarkerAlt /> {property.locality || 'Unknown Locality'},{' '}
