...
const components = {
  ...
  FirstBlock: dynamicImport(
    () => import("@/app/components/project/firstBlock")
  ),
  Overview: dynamicImport(() => import("@/app/components/project/overview")),
...
export default async function page({ params }: Props) {
  ...
  const { basicData: data } = projResponse;
    ...
    <section className="w-full relative break-words ">
      ...
      <div className="mt-[70px] sm:mt-[90px] w-full sm:pb-[2%] flex xl:text-ellipsis items-center justify-center flex-col ">
        ...
        <components.MobileHidden>
          <components.Navigation
            isBrochure={
              ...
              projResponse.phaseOverview?.some(
                (item: any) => item.phaseBrochureUrl
              )
            }
            detailsData={{
              ...data,
              nearByLocations: projResponse.nearByLocations,
            }}
            slug={slug}
          />
        </components.MobileHidden>
        <components.Overview
          {...data}
          slug={slug}
          PhaseOverview={projResponse.phaseOverview}
        />
        ...
        <components.ProjectDetailsP
          ...
          projData={data}
          PhaseOverview={projResponse.phaseOverview}
          isPartialData={data.partialUnitData ?? false}
        />
        ...
        {data.lat && data.lang && (
          <components.LeafMap
            ...
            projName={data.projectName}
            type="proj"
            mapData={projResponse.nearByLocations}
          />
        )}
        <components.ProjectBrouchersSection
          projName={data.projectName}
          phaseOverviewData={projResponse.phaseOverview}
        ...
        {refURls && refURls.length > 0 && (
          <components.Disclamer refUrls={refURls} />
        )}
...
export async function generateMetadata(
  ...
  const configurations = phaseOverview
    .flatMap(
      (phase: { propTypeOverview: Record<string, { unitTypes: string[] }> }) =>
        Object.values(phase.propTypeOverview).flatMap((type) => type.unitTypes)
    )
    .filter(
  ...
  const nearbyLandmarks = [
    ...(nearByLocations.school || [])
      .slice(0, 2)
      .map((s: { name: string }) => s.name),
    ...(nearByLocations.hospital || [])
      .slice(0, 2)
