...
function PageTitle({}: Props) {
      ...
      if (paramsData && Object.keys(paramsData).length > 0) { 
        if(paramsData.slug){
            ...
            const pageTitle = `Search Results For ${
                ...
                state.cg !== "S" ? "Rent" : "Sale" } in ${
                paramsData.project ? paramsData.project : ""} ${
    ...
    return (
        <h1 className="font-bold text-[16px] md:text-[18px] xl:text-[20px] mb-[6px] ml-[8px] capitalize ">{getTitle(path)}</h1>
    )
}
