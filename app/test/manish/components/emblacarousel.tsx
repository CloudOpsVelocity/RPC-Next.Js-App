...
const EmblaCarousel: React.FC<PropType> = (props) => {
  ...
  }, [emblaMainApi, onSelect, selectedIndex]);
  ...
  return (
    <div className="embla">
          ...
          {slides.map((index) => (
            <div className="embla__slide" key={index}>
              <div className="embla__slide__number">
                <Image width={100} height={100} src={dumArray[index]} alt="" />
              </div>
            </div>
          ))}
        </div>
      </div>
      <button onClick={() => handleSideButtons("prev")}>prev</button>
      ...
      <button onClick={() => handleSideButtons("next")}>next</button>
    </div>
  );
};
