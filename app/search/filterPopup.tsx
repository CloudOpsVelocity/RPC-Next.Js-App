...
const FilterPopup = () => {
                ...
                {searchDetails.map((eachItem, index) => {
                    return(
                        <Button 
                            ...
                            buttonClass={` whitespace-nowrap w-full text-[12px] flex flex-row-reverse  justify-end pl-[10%] items-center border-solid border-b-[0.5px] items-start font-[500] p-[2%] h-[31px] gap-[8px] ${current == eachItem ? "text-[#148B16] bg-[#F1F9FF]" : "text-[#202020] bg-[#FCFCFC] "} `}
    ...
    {/* Right Side Fields Con */}
            <div className="w-[80%] p-[1%] pl-[2%] max-h-[400px] overflow-y-auto pb-[5%] ">
                <h3 className=" text-[#202020] mb-[2%] text-[14px] font-[500] ">Property Status</h3>
                    ...
                    {locality.map((eachLocality, index)=>{
                        return(
                            <div key={index} className='flex justify-center items-center p-[1%] rounded-[10px] border-[#92B2C8] border-solid border-[1px]  '>
                                {eachLocality}
                                <span className='cursor-pointer' onClick={()=>removeLocality(index)}>
                                    {miniItemsCrossIcon}
                                </span>
                            </div> 
                        )
