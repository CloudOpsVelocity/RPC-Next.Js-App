...
export default function NewAddedProjects({ data, shortIds ,cityId}: Props) {
  ...
  return (
    ...
    (isEnabled ? newlyAddedProjects.status && newlyAddedProjects['New Launch'].length : data.length) > 0 &&  <div className="mt-[40px] sm:mt-[60px] w-[95%] m-auto">
      <MainHeading
        title="Featured Projects"
        content="Premier Real Estate Projects Awaiting You"
      />
      
      <CardCarousel data={isEnabled ? newlyAddedProjects['New Launch'] : data} shortIds={shortIds}  />
    </div>
  );
}
