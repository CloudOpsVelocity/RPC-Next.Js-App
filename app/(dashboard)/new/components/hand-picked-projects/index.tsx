...
export default function HandPickedProjects({ data, shortIds }: Props) {
  ...
  const url = 
  active == 0 ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/search?projStatus=108` : active == 1 ?` ${process.env.NEXT_PUBLIC_BACKEND_URL}/search?projStatus=106 `: `${process.env.NEXT_PUBLIC_BACKEND_URL}/search?projStatus=107` ;
      ...
      <div className=" w-[95%]">
        ...
        <div className=" w-full flex flex-col xl:flex-row justify-center items-start mt-2 xl:mt-10 flex-nowrap sm:gap-6 ">
          ...
          <div className=" max-w-[600px] sm:max-w-[1466px] mt-[16px] sm:mt-0 ">
            <CardCarousel
              data={data.featured.filter((each:any)=> each.projstatus == config[active])}
              active={active}
              shortIds={shortIds}
            />
          </div>
