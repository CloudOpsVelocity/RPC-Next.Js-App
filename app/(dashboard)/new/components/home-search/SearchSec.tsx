...
{/*       <Combobox.Target>
        <PillsInput classNames={{ input: classes.homePageSearch }} size={isTab ? "xs" : "md"} w={"100%"}>
          ...
          <div onClick={() => setShowAllLocalities(!showAllLocalities)}  className="  p-2 gap-2 xl:gap-[8px] pl-2 xl:pl-[8px] max-w-full flex items-center justify-start  flex-wrap">
          {" "}
          {f.city && (
              <Pill
                className="capitalize !text-[12px] !sm:text-[14px] "
          ...
          {f.locality?.map((each, index) => (
            ...
            {f.locality?.length > (isTab ? 1 : 2) &&
              !showAllLocalities && f.locality?.length > (isTab ? 1 : 2)&&(
                <Pill
